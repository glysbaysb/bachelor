...
\paragraph{Die Schnittstelle zwischen Controller und simulierter Welt} ist \textit{callback} basiert.
\begin{lstlisting}[frame=single, language=c++] 
void* connectToWorld(char* url);
int registerCallback(void* world, TypeGetWorldStatus callback);
// void (_cdecl* TypeGetWorldStatus)(struct WorldStatus w);
\end{lstlisting}
