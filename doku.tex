...
\maketitle
\todo{Bilder: vom Fehlerinjektor?, Klassendiagram?, Ablaufdiagram für Algo?, Konsens?, Voter?}
\thispagestyle{empty}
...
\paragraph{Simulation} Die simulierte Welt besteht aus einer kreisf{\"{o}}rmigen Platte. Auf dieser k{\"{o}}nnen bis zu \gls{N} Roboter und $1$ Ladestation
...
an der Ladestation aufLadeen. Auch wenn die Roboter sich nicht bewegen, verbrauchen sie Energie (allerdings weniger
schnell).

Gesteuert werden die Roboter von bis zu \gls{M} $(M \geq N)$ Controllern. Einer oder mehr Controller stimmen sich ab,
so dass:
\begin{itemize}
\item die Roboter ihre Energie immer rechtzeitig aufLadeen
...
Beispielimplementation, die die oben ausgef{\"{u}}hrten Anforderungen erf{\"{u}}llen. Dies wurde auch mithilfe eines Fehlerinjektors verifiziert.
...
ungeeignet machen. Es gibt auch wenig Interaktionsm{\"{o}}glichkeiten mit dem System, dadurch ist es schwer etwas auszuprobieren und
...
\begin{itemize}
\item die Kollisionserkennung, einerseits um zu bestrafen wenn Roboter in einander fahren und auch zur Erkennung des Kontaktes mit der Ladestation
...
der Objekte \footnote{Dies beinhaltet nicht nur die Roboter und Ladestationen sondern auch die Kamera oder statische Objekte wie Mauern.} auszulesen und zu manipulieren.

Beispielsweise wurde der Ladestation ein Skript zugeordnet das solange ein Objekt mit dieser kollidiert testet ob dies Objekt ein Roboter ist, und falls dies der Fall ist,
den Roboter aufladen. Ein anderes Skript bewegt die Kamera abh{\"{a}}ngig von den Tastatureingaben des Benutzers.
...
dann aber "heimlich" die C++ Runtime hinzu. Eine rein in C geschriebene Bibliothek ist daher vorzuziehen. \todo{bla bla exceptions}
...
\paragraph{Fixed Size String} Um kleinere Zeichenketten zu encodieren gibt es erst ein Signalzeichen \todo{besseres wort} in dem die obersten 3 bits gesetzt sind. Die restlichen 5bits geben die L{\"{a}}nge der Zeichenkette an, wodurch diese auf $2^5 - 1 = 31$ Zeichen begrenzt werden. Dies f{\"{u}}hrt zu einem durchschnittlichen Overhead von $ \frac{1}{\frac{2^5-1}{2}} = 6\% $.

L{\"{a}}ngere Zeichenketten
werden mit anderen Signalzeichen kodiert --- beispielsweise 0xDB f{\"{u}}r eine Zeichenketten mit der Maximall{\"{a}}nge von $(2^32)-1$ Zeichen.
\footnote{L{\"{a}}ngen werden immer in big-endian kodiert um den Datenaustausch auch zwischen Rechnern unterschiedlicher Architekturen gew{\"{a}}rleisten zu k{\"{o}}nen}. In diesem Fall folgt die L{\"{a}}ng als 32bit big endian Wert mit fester Breite. Also ein verschwindend geringer durchschnittlicher Overhead von $ \frac{1}{\frac{2^32-1}{2}} = 0.00000004\% $.
...
\clearpage
\section{Die Welt}
Die simulierte Welt besteht aus den Robotern, die von den Studierenden gesteuert werden sollen, einer Ladestation, an der die Robter Energie laden k{\"{o}}nnen, und
...
ein Kegel, an dessen DINGSBUMS\todo{} befinden sich Ausbuchtungen, diese geben eine visuelle Indikation {\"{u}}ber die Position der R{\"{a}}der. Der Ladestand wird {\"{u}}ber eine Lampe angezeigt, die ihre
...
Das Gewicht des Roboters ist abh{\"{a}}ngig vom Grundgewicht des Roboters und seinem momentanen Ladestatus: 
\begin{equation}
    \label{eq:w}
 w(N_i) = 1 + e(N_i) * 0.03
\end{equation}
...
Die Platte soll sich im Gleichgewicht befinden, also sich nicht bewegen und nicht gekippt sein. Dies ist nur der Fall wenn alle Kr{\"{a}}fte/Gewichte (der Objekte, also der Ladestation und aller Roboter) die auf die Platte wirken, sich zu Null aufaddieren. Dabei werden die Objekte als Vektoren bestehend aus Gewicht und Abstand zum Mittelpunkt betrachtet.
...
ist und gleichzeitig die Roboter nicht ihre ganze Energie verbrauchen. Gleichzeitig sollten die Roboter auch nicht miteinander
kollidieren.
...
unkritisch oder dazwischen? Dabei ist das Kriterium die momentane Ladeung geteilt durch die Entfernung zur Ladestation. Roboter mit nur wenig Energiereserve
versuchen also sich aufzuLadeen, fast volle Roboter versuchen auszubalancieren.
