...
Auch mit gr{\"{o}}sstm{\"{o}}glicher Sorgfalt ist es nicht m{\"{o}}glich fehlerfreie Software zu
entwickeln.\todo{jpl} Es kann allerdings versucht werden die Auswirkungen der verbleibenden Fehler zu
minimieren. Das System soll also fehlertolerant gemacht werden. Zu diesem Zweck gibt es eine Vielzahl an
Techniken; hier soll nur beispielhaft die Redudanz genannt werden: Wenn ein System eine Funktionalit{\"{a}}t
immer bereitstellen muss, kann es sinnvoll sein das ganze System zu duplizieren. Falls dann die erste
Instanz ausf{\"{a}}llt kann statdessen die zweite genutzt werden, womit die Funktionalit{\"{a}}t gew{\"{a}}hrleistet
bleibt. Dies ist zum Beispiel bei Passagierflugzeugen der Fall, der Ausfall eines Motors f{\"{u}}hrt nicht
zur Katastrophe da der zweite auch alleine erm{\"{o}}glicht das Flugzeug sicher zu landen. Mit Redudanz
alleine k{\"{o}}nnen aber nicht alle Probleme gel{\"{o}}st werden. Beispielsweise kann man sich vorstellen
das Patientendaten auf mehreren Rechner abgespeichert werden, um auch bei Ausfall eines Rechners trotzdem
noch nachvollziehen zu k{\"{o}}nen welche Medikamente die Patienten brauchen. Was ist aber wenn es zu
einem Patienten in den verschiednen Datenbanken verschiedene Angaben stehen? Welche Informationen sind die
richtigen?
...
bla bla gibt gaÂ´r keine NextStep rechner mehr.
...
Um aus den Kr{\"{a}}ften den Kippwinkel zu errechnen muss, wie im Bild dargestellt, der Arcussinus genutzt werden. Dabei ist die "Hypothenuse" das Plattengewicht ($ N_Platte = 1000$). \todo{das wirkt total komisch. Ich teile Kraft/Gewicht? Und sowieso kann man asinus fuer solche Sachen ueberhaupt benutzen?}
