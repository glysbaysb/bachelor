...
\maketitle
\todo{Bilder: Systemaufbau, von der Simulation selbst, vom Fehlerinjektor?, Klassendiagram?, Ablaufdiagram für Algo?, Konsens?}
\thispagestyle{empty}
...
\paragraph{Motivation} Im Rahmen des Modules Ausfallsichere Systeme sollen die Studenten ihr neu erworbenes Wissen auch praktisch anwenden k{\"{o}}nnen. \todo{bla bla verdeutlichen, vertiefen bla bla}
...
Kontrollern, Votern, Simulation und den 2 Netzwerken?}
...
Um die, sehr grosse, Auswahl einzuschr{\"{a}}nken wurden die nachfolgenden Kriterien benutzt.
...
Die Grundpidee von msgpack ist es Datentypen nicht mit fester Breite sondern nur mit der minmal n{\"{o}}tigen Breite (oder wenigstens einer Ann{\"{a}}hrung an diese) abzuspeichern.
...
programmiert werden das sie mit \textit{value faults/message corruption}\cite{richling-ass} umgehen k{\"{o}}nnen.
...
\paragraph{Crash failure} Die Controller k{\"{o}}nnen jederzeit ausfallen. Es k{\"{o}}nnen bis zu \todo{...} aller Controller ausfallen.
Die Voter sind, per Definition, gegen diese Art von Fehlern unempfindlich.

\paragraph{Value error} Das Netzwerk kann Pakete verf{\"{a}}lschen, es wird angenommen das bei bis zu $\frac{1}{20}$ aller Pakete Verf{\"{a}}lschungen geben kann, diese sich allerdings auf \textit{Single Byte Errors} beschr{\"{a}}nkt.
...
Es wird davon ausgegangen das die Controller sehr fehleranf{\"{a}}lig sind und leicht ausfallen - daraus folgt das eine gro{\ss}e
...
sind kann eine ordnungsgem{\"{a}}sse Steuerung nicht mehr garantiert werden.
...
\begin{itemize}
\item Durch expermientelle Verifikation wurde klar das im Netzwerk nur 1 Byte pro Paket verf{\"{a}}lscht wird. Da das durchschnittliche Paket um mindestens Faktor
	100 gr{\"{o}}ser ist, ist nicht erforderlich eine sehr kompakte Kodierung zu benutzen, es ist ausreichend die Redudanz zu reduzieren. 
