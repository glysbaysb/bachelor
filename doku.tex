...
ungeeignet machen. Dies wird noch dadurch verst{\"{a}}rkt das es wenig Interaktionsm{\"{o}}glichkeiten mit dem System gibt.
...
$ N_i = \bigl(\begin{smallmatrix} x(i) \\ y(i) \\ w(i) \end{smallmatrix}\bigr)$, einen
Geschwindigkeitsvektor
$ V_i = \bigl(\begin{smallmatrix} \Delta x \\ \Delta y \end{smallmatrix}\bigr)$ und einen Rotationsquaternion
...
Der Robot kann sich nur in "Fahrrichtung" (angegben durch $R_i$) bewegen und erreicht dabei eine
...
\subsection{Netzwerk}
Alle UDP Pakete k{\"{o}}nnen verf{\"{a}}lscht werden. Die einzelen Pakete werden dabei so manipuliert das, durchschnittlich, in $ \frac{1}{20} $ aller Pakete
ein Byte verf{\"{a}}lscht wird, das Betriebssystem diese Modifikation allerdings nicht erkennt.
...
Dadurch wird ein crash failure simuliert.
...
(255, 240) RS Codes k{\"{o}}nnen $n$ aus $m$ Einzelfehler und bis zu $x$ Doppelfehler pro Nachricht erkannt werden. 
...
\item Durch expermientelle Verifikation wurde klar das im Netzwerk nur 1 Byte pro Paket verf{\"{a}}lscht wird. Da das durchschnittliche Paket um mindestens Faktor
	10 gr{\"{o}}ser ist, ist nicht erforderlich eine sehr kompakte Kodierung zu benutzen, es ist ausreichend die Redudanz zu reduzieren. 
