...
Da die Simulation mit einer, nicht vorher bestimmten, Anzahl von Votern/Robotern, die nicht unbedingt im gleichen \textit{subnet} sein m{\"{u}}ssen ist das Publish-Suscribe Kommunikationspattern\cite{pubsub}
...
sich verbinden k{\"{o}}nnen.
Es gibt allerdings keine Netzwerklibrary die dieses Pattern und RPC anbietet, da es einfach ist auf Publish-Suscribe ein RPC Modell aufzubauen, als dieses Pattern, zum Beispiel, grpc beizubringen, wird
...
eines fehlerkorrigierenden Codes, so wenig Redudanz wie m{\"{o}}glich aufzuweisen und damit nur wenig f{\"{u}}r Paketverf{\"{a}}lschungen nur wenig anf{\"{a}}llig zu sein.
