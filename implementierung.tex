...
Ein weiterer Bestandteil der Fehlerinjektion ist das Verf{\"{a}}lschen der Weltstatusinformationen. Um die Funktionalit{\"{a}}t
zu testen wird die durchschnittliche Dauer zwischen dem Empfangen der Weltstatusinformationen errechnet, w{\"{a}}hrend die
Fehlerwahrscheinlichkeit des "fallen lassen" einer Weltstatusinformationen erh{\"{o}}ht wird.

\begin{figure}
	\centering
	\includevisio[width=\textwidth]{dzzdw}
	\caption{Durchschnittliche Zeit zwischen dem Empfangen von Weltstatusinformationen}
	\label{fig:zemsf}
\end{figure}

Der Erwartungswert ist 100ms, denn dies wird in der Konfigurationsdatei so angegeben. Allerdings kann dieser Wert nicht
erreicht werden --- bei einer Fehlerwahrscheinlichkeit von 0, liegt die durchschnittliche Zeit umgef{\"{a}}hr 10\% {\"{u}}ber
dem erwarteten Wert. Dies ist aber nicht verwunderlich, schlie{\ss}lich l{\"{a}}uft die Simulation auf normalen Rechner, welche
auch andere Aufgaben abarbeiten wordurch die Echtzeitf{\"{a}}higkeit nicht gew{\"{a}}hrleistet werden kann (und auch gar nicht
gew{\"{u}}nscht ist).

Nicht destotrotz ist es ersichtlich das mit steigender Fehlerwahrscheinlichkeite auch die durchschnittliche Dauer zum Empfangen
von Weltstatusinformationen steigt. Das bedeutet das die Fehlerinjektion wirkt.
...
Allerdings ist es n{\"{o}}tig die Konstanten etwas anzupassen um das physikalische Verhalten der Simulation zu verbessern.
In diesem Bereich sind auch weitere Verbesserungsm{\"{o}}glichkeiten denkbar, zum Beispiel eine Anpassung der Gravitationskraft
oder eine realistischere Auf-/Entladekurve.
