...
Die Parameter f{\"{u}}r die Fehlerinjektion k{\"{o}}nnen in der Datei `fault_injector.json` eingestellt werden. Dort ist es m{\"{o}}glich f{\"{u}}r die einzelnen Motoren
...
$\omega_t$ und $\delta$, auch der Radius der Platte, der Maximalkippwinkel \todo{XXXX} eingestellt werden.

\section{Der Code inklusive Interface}
Der restliche Code ist in einem git repository und wird mithilfe CMakes gebaut. Dabei gibt es verschiedene Abh{\"{a}}nigkeiten, diese sind als in git als Untermodule eingebunden.
Die meisten Libraries kompilieren und k{\"{o}}nnen ohne Probleme gelinkt werden, allerdings muss bei googletest aufgepasst werden das googletest und die Testprogramme mit den
...
wird als Parameter nur das Interface {\"{u}}bergeben.
