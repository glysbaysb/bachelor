...
\Rightarrow \frac{1}{X} = \frac{Max}{30 * Curr} \\
\Rightarrow X = \frac{30 * Curr}{Max}
\end{equation}
...
\begin{equation}\label{eq:schwingung}
	s(t) = s(0) e^\mathrm{-\delta t} sin(\omega_d t)
\end{equation}
\footnote{$\delta$ und $\omega_d$ sind in der Simulation frei einstellbar}

Hierbei ist $s(t)$ ein Vektor, in dem die Geschwindigkeits{\"{a}}nderung in X- und Y-Richtung die jeweiligen Elemente sind. Um die gesamte Schwindung der
